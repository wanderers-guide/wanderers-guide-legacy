        ...
        if (currentUser) {
          ...
          User.create({
            ...
            .catch((err) => {
              console.error(err);
              console.error(
                "Failed to create user from Google profile data, using default data instead...",
              );
              User.create({
                googleID: profile.id,
                redditID: "",
                username: "New User",
                thumbnail:
                  "https://wanderersguide.app/images/fb_profile_pic.png",
              }).then((defaultNewUser) => {
  ...
  new RedditStrategy(
    {
      clientID:
        process.env.NODE_ENV === "production"
          ? keys.reddit.clientID
          : keys.reddit_dev.clientID,
      clientSecret:
        process.env.NODE_ENV === "production"
        ...
        if (currentUser) {
          ...
          User.create({
            ...
            .catch((err) => {
              console.error(err);
              console.error(
                "Failed to create user from Reddit profile data, using default data instead...",
              );
              User.create({
                googleID: "",
                redditID: profile.id,
