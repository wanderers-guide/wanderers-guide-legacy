...
function getUserCampaignLimit() {
  return 1;
}
module.exports = class CampaignGathering {
  static canMakeCampaign(user, campaigns) {
    return (
      user.isPatreonMember === 1 || campaigns.length < getUserCampaignLimit()
    );
  }
  ...
  static getCampaignDetailsFromChar(charID) {
    return CampaignAccessToken.findOne({
      ...
      return CampaignGathering.getCampaignDetails(token.campaignID).then(
        (details) => {
          return details;
        },
      );
  ...
  static getUsersInCampaignFromChar(charID) {
    return CampaignAccessToken.findOne({
      ...
      return CampaignGathering.getUsersInCampaign(token.campaignID).then(
        (userIDs) => {
          return userIDs;
        },
      );
