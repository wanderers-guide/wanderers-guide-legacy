...
function getRandomClientID() {
  /* Isn't a great, but good enough for now */
  return (
...
module.exports = class ClientAPI {
  static getRequestAccessData(clientID, charID, accessRights) {
    return APIClientApp.findOne({ where: { clientID: clientID } }).then(
      (apiClient) => {
        if (apiClient.accessRights !== accessRights) {
          return null;
        }
  ...
  static hasAccessToCharacter(charID, accessToken) {
    return AccessToken.findOne({
      where: { charID: charID, accessToken: accessToken },
    }).then((accessTokenModel) => {
      ...
      if (
        ...
        let expireDate = new Date(
          accessTokenModel.createdAt.getTime() +
            getTokenExpirationLength() * 1000,
        );
  ...
  static getClientsWhoAccess(charID) {
    return AccessToken.findAll({ where: { charID: charID } }).then(
      (accessTokens) => {
        let clientPromises = [];
        ...
        return Promise.all(clientPromises).then(function (apiClients) {
          return apiClients;
        });
      },
    );
  ...
  static validAPIKey(apiKey) {
    return APIClientApp.findOne({ where: { apiKey: apiKey } }).then(
      (apiClient) => {
  ...
  static getNumberOfClients(userID) {
    return APIClientApp.count({ where: { userID: userID } }).then(
      (clientsNum) => {
        return clientsNum;
      },
    );
  ...
  static getAllClients(socket) {
    ...
    if (userID != -1) {
      return APIClientApp.findAll({ where: { userID: userID } }).then(
        (apiClients) => {
          return apiClients;
        },
      );
  ...
  static addClient(socket, clientData) {
    let userID = getUserID(socket);
    if (userID != -1) {
      return ClientAPI.getNumberOfClients(userID).then((clientsNum) => {
