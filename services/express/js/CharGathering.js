...
module.exports = class CharGathering {
  static getAllMetadata(userID, charID) {
    return CharDataMappingModel.findAll({ where: { charID: charID } }).then(
      (charMetaDatas) => {
  ...
  static getCalculatedStats(userID, charID) {
    return CalculatedStat.findOne({ where: { charID: charID } }).then(
      (calculatedStats) => {
  ...
  static getSourceBooks(userID, enabledSources, enabledHomebrew) {
    return Book.findAll({
      ...
      return UserHomebrew.getCollectedHomebrewBundles(userID).then(
        (hBundles) => {
    ...
    return ClassArchetype.findAll({
      where: {
        ...
        [Op.not]: [
          {
            contentSrc: {
              [Op.or]: TempUnpublishedBooks.getSourcesArray(userID),
            },
          },
        ],
      },
  ...
  static async getAllClassFeatureOptions(
    userID,
    enabledSources,
    enabledHomebrew,
  ) {
  ...
  static getAllExtraClassFeatures(userID, charID) {
    return CharDataMapping.getDataAll(charID, "classAbilityExtra", null).then(
      (extraClassFeaturesDataArray) => {
        let promises = [];
        for (let data of extraClassFeaturesDataArray) {
          ...
          promises.push(
            Prisma.classAbilities.findUnique({
              where: { id: parseInt(data.featureID) },
            }),
          );
        }

        return Promise.all(promises).then(function (classFeatures) {
          for (let data of extraClassFeaturesDataArray) {
  ...
  static getInventory(userID, inventoryID) {
    return Inventory.findOne({ where: { id: inventoryID } }).then(
      (inventory) => {
        if (inventory == null) {
          return {};
        }
  ...
  static async getAllSkills(
    ...
    if (loreDataArray == null) {
      loreDataArray = await CharDataMapping.getDataAll(
        charID,
        "loreCategories",
        null,
      );
    ...
    for (const loreData of loreDataArray) {
      ...
      if (loreName != null) {
        ...
        skillArray.push({
          SkillName: capitalizeWords(loreName) + " Lore",
          Skill: loreSkillCopy,
        });
      }
    }
          ...
          return AncestryLanguage.findAll({
            ...
            return AncestryBoost.findAll({
              where: {
                homebrewID: {
                  [Op.or]:
                    CharContentHomebrew.getHomebrewArray(enabledHomebrew),
                },
              },
            }).then((ancestBoosts) => {
              return AncestryFlaw.findAll({
                ...
                return Tag.findAll({
                  ...
                  return CharGathering.getAllSenses(userID).then(
                    (senseTypes) => {
                      return CharGathering.getAllPhysicalFeatures(userID).then(
                        (physicalFeatures) => {
                          ...
                          for (const ancestry of ancestries) {
                            ...
                            for (let senseType of senseTypes) {
                              if (senseType.id === ancestry.visionSenseID) {
                                ...
                                senseType.id === ancestry.additionalSenseID
                              ) {
                                additionalSense = senseType;
                              }
                            }
                            let physicalFeatureOne,
                              physicalFeatureTwo = null;
                            for (let physicalFeature of physicalFeatures) {
                              if (
                                physicalFeature.id ===
                                ancestry.physicalFeatureOneID
                              ) {
                                physicalFeatureOne = physicalFeature;
                              } else if (
                                physicalFeature.id ===
                                ancestry.physicalFeatureTwoID
                              ) {
                                physicalFeatureTwo = physicalFeature;
                              }
                            }
                            ancestryMap.set(ancestry.id, {
                              Ancestry: ancestry,
                              Heritages: [],
                              Languages: [],
                              BonusLanguages: [],
                              ...
                              PhysicalFeatureOne: physicalFeatureOne,
                              PhysicalFeatureTwo: physicalFeatureTwo,
                            });
                          }
                          ...
                          for (const heritage of heritages) {
                            ...
                            if (heritage.ancestryID != null) {
                              ...
                              let ancestry = ancestries.find((ancestry) => {
                                return (
                                  ancestry.isArchived == 0 &&
                                  ancestry.name === heritage.indivAncestryName
                                );
                              });
                              if (ancestry != null) {
                          ...
                          for (const ancestLang of ancestLangs) {
                            if (ancestLang.isBonus === 1) {
                              ...
                              let ancestryStruct = ancestryMap.get(
                                ancestLang.ancestryID,
                              );
                              if (ancestryStruct != null) {
                          ...
                          for (const ancestBoost of ancestBoosts) {
                            let ancestryStruct = ancestryMap.get(
                              ancestBoost.ancestryID,
                            );
                            if (ancestryStruct != null) {
                              ancestryStruct.Boosts.push(
                                ancestBoost.boostedAbility,
                              );
                            }
                          }
                          ...
                          for (const ancestFlaw of ancestFlaws) {
                            let ancestryStruct = ancestryMap.get(
                              ancestFlaw.ancestryID,
                            );
                            if (ancestryStruct != null) {
                              ancestryStruct.Flaws.push(
                                ancestFlaw.flawedAbility,
                              );
                            }
                          }
  ...
  static getAllLanguages(userID, charID) {
    return Character.findOne({ where: { id: charID } }).then((character) => {
      return AncestryLanguage.findAll({
        where: { ancestryID: character.ancestryID },
      }).then((ancestLangs) => {
        return Language.findAll({
          where: {
            homebrewID: {
              [Op.or]: CharContentHomebrew.getHomebrewArray(
  ...
  static async getResistancesAndVulnerabilities(
    userID,
    charID,
    ...
    if (resistancesDataArray == null) {
      resistancesDataArray = await CharDataMappingExt.getDataAllResistance(
        charID,
      );
    }

    if (vulnerabilitiesDataArray == null) {
      vulnerabilitiesDataArray =
        await CharDataMappingExt.getDataAllVulnerability(charID);
    }

    return {
      Resistances: resistancesDataArray,
      Vulnerabilities: vulnerabilitiesDataArray,
    };
  }

  static async getNoteFields(userID, charID) {
    let notesDataArray = await CharDataMapping.getDataAll(
      charID,
      "notesField",
      null,
    );
  ...
  static getNoteField(userID, charID, notesData) {
    let noteFieldID = srcStructToCode(charID, "notesField", notesData);
    return NoteField.findOne({
      where: { id: noteFieldID, charID: charID },
    }).then((noteField) => {
      if (noteField != null) {
        notesData.text = noteField.text;
        notesData.placeholderText = noteField.placeholderText;
  ...
  static async getSpecializations(
    ...
    if (weapSpecialsDataArray == null) {
      weapSpecialsDataArray = await CharDataMapping.getDataAll(
        charID,
    ...
    if (weapCriticalsDataArray == null) {
      weapCriticalsDataArray = await CharDataMapping.getDataAll(
        charID,
    ...
    if (armorSpecialDataArray == null) {
      armorSpecialDataArray = await CharDataMapping.getDataAll(
        charID,
        "armorSpecialization",
        null,
      );
  ...
  static async getSpellData(userID, charID) {
    ...
    let spellBookArray = spellBookSlotArray.concat(
      spellBookFocusArray.filter(
        (entry) => spellBookSlotArray.indexOf(entry) < 0,
      ),
    );

    // Innate Spells //
    let innateSpellArray = await CharDataMappingExt.getDataAllInnateSpell(
      charID,
    );
    ...
    for (const innateSpell of innateSpellArray) {
      ...
      let newPromise = InnateSpellCasting.findOrCreate({
        where: { innateSpellID: innateSpellCastingsID },
        defaults: { timesCast: 0 },
        raw: true,
      });
  ...
  static getChoicesAbilityBonus(userID, charID) {
    return CharDataMappingExt.getDataAllAbilityBonus(charID).then(
      (bonusDataArray) => {
  ...
  static getChoicesFeats(userID, charID) {
    return CharDataMapping.getDataAll(charID, "chosenFeats", Feat).then(
      (featDataArray) => {
  ...
  static getChoicesDomains(userID, charID) {
    return CharDataMapping.getDataAll(charID, "domains", Domain).then(
      (domainDataArray) => {
  ...
  static async getAncestry(userID, charID, character = null) {
    ...
    if (character.ancestryID != null) {
      return await Prisma.ancestries.findUnique({
        where: { id: character.ancestryID },
      });
    } else {
  ...
  static async getBackground(userID, charID, character = null) {
    ...
    if (character.backgroundID != null) {
      return await Prisma.backgrounds.findUnique({
        where: { id: character.backgroundID },
      });
    } else {
      return Promise.resolve();
    }
  ...
  static async getHeritage(userID, charID, character = null) {
    ...
    if (character.heritageID != null) {
      return await Prisma.heritages.findUnique({
        where: { id: character.heritageID },
      });
    } else if (character.uniHeritageID != null) {
      return await Prisma.uniHeritages.findUnique({
        where: { id: character.uniHeritageID },
      });
    } else {
      return Promise.resolve();
    }
  ...
  static async getClass(
    ...
    let cClass = await Prisma.classes.findUnique({
    ...
    if (charID != null) {
      ...
      keyBoostData = await CharDataMappingExt.getDataSingleAbilityBonus(
        charID,
        srcStruct,
      );
    }
    ...
    if (keyBoostData != null) {
      keyAbility = keyBoostData.Ability;
    }
    ...
    if (cClass != null) {
      const classAbilities = await ClassAbility.findAll({
        ...
        where: {
          ...
          [Op.or]: [{ classID: cClass.id }, { indivClassName: cClass.name }],
        },
        raw: true,
      });
  ...
  static getAncestryLanguages(userID, ancestryID) {
    return AncestryLanguage.findAll({ where: { ancestryID: ancestryID } }).then(
      (ancestLangs) => {
        return ancestLangs;
      },
    );
    ...
    return Character.findOne({ where: { id: charID } }).then((character) => {
      return ClassAbility.findOne({
        where: {
          name: featureName,
          contentSrc: {
            [Op.or]: CharContentSources.getSourceArray(
  ...
  static getCharAnimalCompanions(userID, charID) {
    return CharAnimalCompanion.findAll({ where: { charID: charID } }).then(
      (charAnimalComps) => {
  ...
  static getCharFamiliars(userID, charID) {
    return CharFamiliar.findAll({ where: { charID: charID } }).then(
      (charFamiliars) => {
  ...
  static async getCompanionData(
    userID,
    charID,
    enabledSources,
    enabledHomebrew,
  ) {
    let allAnimalCompanions = await CharGathering.getAllAnimalCompanions(
      userID,
      enabledSources,
      enabledHomebrew,
    );

    let charAnimalComps = await CharGathering.getCharAnimalCompanions(
      userID,
      charID,
    );

    let allSpecificFamiliars = await CharGathering.getAllSpecificFamiliars(
      userID,
      enabledSources,
      enabledHomebrew,
    );

    let allFamiliarAbilities = await CharGathering.getAllFamiliarAbilities(
      userID,
      enabledSources,
      enabledHomebrew,
    );
  ...
  static async getAbilityScores(
    ...
    if (charAbilityScores == null) {
      charAbilityScores = await CharGathering.getBaseAbilityScores(
        userID,
        charID,
      );
    }
  ...
  static getProfs(userID, charID) {
    return CharDataMappingExt.getDataAllProficiencies(charID).then(
      (profDataArray) => {
        let profMap = new Map();
        for (const profData of profDataArray) {
          // Convert lores to be the same
          if (profData.To.includes("_LORE")) {
            profData.To = capitalizeWords(
              profData.To.replace("_LORE", " Lore"),
            );
          }
  ...
  static getFinalProfs(userID, charID) {
    return CharGathering.getProfs(userID, charID).then((profMap) => {
      ...
      for (const [profName, dataArray] of profMap.entries()) {
        ...
        finalProfMap.set(
          profName,
          profTypeFromNumber(highestNumUps + increaseNumUps),
        );
      }
  ...
  static getClassArchetypeID(userID, charID) {
    ...
    return CharDataMapping.getDataSingle(
      charID,
  ...
  static getBaseAbilityScores(userID, charID) {
    ...
    return CharDataMappingExt.getDataSingleAbilityBonus(charID, srcStruct).then(
      (bonusData) => {
