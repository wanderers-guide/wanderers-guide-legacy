...
function findSIDFromSlotData(spellSlotDataArray) {
  if (spellSlotDataArray.length == 0) {
    return null;
  }
...
module.exports = class CharSpells {
  ...
  static addToSpellBook(
    ...
    let createSpellBookSpell = function (
      charID,
      spellSRC,
      spellID,
      spellLevel,
  ...
  static addToSpellBookFromBuilder(
    charID,
    spellSRC,
    spellID,
    spellLevel,
  ...
  static getSpellBook(charID, spellSRC, isFocus) {
    return SpellBookSpell.findAll({
      ...
      return CharSpells.getSpellList(charID, spellSRC).then((spellList) => {
        return CharSpells.getSpellCastingType(charID, spellSRC).then(
          (spellCastingType) => {
            return CharDataMapping.getDataAll(
              charID,
              "spellKeyAbilities",
              null,
            ).then((spellKeyAbilityDataArray) => {
              let keyAbility = getKeyAbilityFromSpellSRC(
                spellKeyAbilityDataArray,
                spellSRC,
              );
              let spellBookArray = [];
              for (let spellBookSpell of spellBookSpells) {
                if (spellBookSpell.spellType == null) {
                  spellBookSpell.spellType = "R:0,G:0,B:0";
                }
                spellBookArray.push({
  ...
  static changeSpellBookSpellType(charID, spellBookSpellID, spellType) {
    ...
    return SpellBookSpell.update(updateValues, {
      where: { id: spellBookSpellID, charID: charID },
    }).then((result) => {
      return;
    });
  }
  ...
  static getSpellList(charID, spellSRC) {
    return CharDataMapping.getDataAll(charID, "spellLists", null).then(
      (spellListDataArray) => {
  ...
  static getSpellCastingType(charID, spellSRC) {
    return CharDataMapping.getDataAll(charID, "spellCastingType", null).then(
      (spellCTypeDataArray) => {
  ...
  static getFocusSpells(charID) {
    return CharDataMapping.getDataAll(charID, "focusSpell", null).then(
      (focusSpellDataArray) => {
  ...
  static getFocusPoints(charID) {
    return CharDataMapping.getDataAll(charID, "focusPoint", null).then(
      (focusPointDataArray) => {
  ...
  static setSpellCasting(
    ...
    return CharDataMapping.getDataSingle(charID, "spellSlots", srcStruct).then(
      (oldSpellSlotsData) => {
        const newSID = getRandomSID();
        let spellSlots = getSpellSlots(newSID, spellcasting);
        if (
          reduceSlotsByOne &&
          (spellcasting == "THREE-QUARTERS" ||
            spellcasting == "FULL" ||
            spellcasting == "REDUCED" ||
            spellcasting == "PARTIAL" ||
            spellcasting == "PSY-REDUCED")
        ) {
          let newSpellSlots = {};
        ...
        if (oldSpellSlotsData != null && oldSpellSlotsData.value != null) {
          ...
          const oldSpellSlotDataArray = [
            ...oldSpellSlotsData.value.match(/\{\"slotID\"\:.+?\}/g),
          ];
          ...
          for (let spellLevel in spellSlots) {
            for (let slotData of spellSlots[spellLevel]) {
              ...
              let oldSlotDataJSON = oldSpellSlotDataArray.find(
                (oldSlotData) => {
                  return (
                    JSON.parse(oldSlotData).slotID - oldSID ==
                    slotData.slotID - newSID
                  );
                },
              );
        ...
        return CharDataMapping.setData(
          ...
          "spellSlots",
          srcStruct,
          spellSRC + "=" + JSON.stringify(spellSlots),
        ).then((result) => {
          return spellSlots;
        });
      },
  ...
  static setSpellSlot(charID, srcStruct, spellSRC, slotLevel, slotType = "") {
    ...
    return CharDataMapping.getDataSingle(charID, "spellSlots", srcStruct).then(
      (spellSlotData) => {
        ...
        if (
          spellSlotData == null ||
          (spellSlotData.value != null &&
            !spellSlotData.value.includes(`"${rowName}":`))
        ) {
          return CharDataMapping.setData(
            charID,
            "spellSlots",
            srcStruct,
            spellSRC + "=" + JSON.stringify(spellSlot),
          ).then((result) => {
  ...
  static changeSpellSlot(charID, updateSlotObject) {
    return CharDataMapping.getDataAll(charID, "spellSlots", null).then(
      (spellSlotDataArray) => {
        ...
        for (const spellSlotData of spellSlotDataArray) {
          let newSpellSlotDataValue = spellSlotData.value.replace(
            re,
            updatedSpellSlotJSON,
          );
          if (newSpellSlotDataValue != spellSlotData.value) {
            ...
            let newPromise = CharDataMapping.setData(
              charID,
              "spellSlots",
              srcStruct,
              newSpellSlotDataValue,
            );
            updateSpellSlotsPromises.push(newPromise);
          }
        }
        return Promise.all(updateSpellSlotsPromises).then(function (result) {
          return;
        });
      },
    );
          ...
          for (const spellSlotData of spellSlotDataArray) {
            ...
            for (let [levelRowName, slotsArray] of Object.entries(spellSlots)) {
              ...
              for (const slotData of slotsArray) {
                if (
                  slotData.level_lock <= character.level &&
                  (slotData.level_cutoff == null ||
                    slotData.level_cutoff > character.level)
                ) {
                  let spellSlotArray = [];
