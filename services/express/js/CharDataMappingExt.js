...
module.exports = class CharDataMappingExt {
  ...
  static getDataAllProficiencies(charID) {
    return CharDataMapping.getDataAll(charID, "proficiencies", null).then(
      (dataArray) => {
  ...
  static getDataAllAbilityBonus(charID) {
    return CharDataMapping.getDataAll(charID, "abilityBonus", null).then(
      (dataArray) => {
        for (let data of dataArray) {
          let vParts = data.value.split(getSeparator());
  ...
  static getDataAllClassChoice(charID) {
    return CharDataMapping.getDataAll(charID, "classChoice", null).then(
      (dataArray) => {
        for (let data of dataArray) {
          let vParts = data.value.split(getSeparator());
  ...
  static getDataAllInnateSpell(charID) {
    return CharDataMapping.getDataAll(charID, "innateSpell", null).then(
      (dataArray) => {
        for (let data of dataArray) {
          let vParts = data.value.split(getSeparator());
  ...
  static setDataResistance(charID, srcStruct, resistType, resistAmount) {
    ...
    return CharDataMapping.setData(charID, "resistance", srcStruct, value).then(
      (result) => {
        return;
      },
    );
  ...
  static getDataAllResistance(charID) {
    return CharDataMapping.getDataAll(charID, "resistance", null).then(
      (dataArray) => {
        for (let data of dataArray) {
          let vParts = data.value.split(getSeparator());
          data.Type = vParts[0];
  ...
  static getDataAllVulnerability(charID) {
    return CharDataMapping.getDataAll(charID, "vulnerability", null).then(
      (dataArray) => {
        for (let data of dataArray) {
          let vParts = data.value.split(getSeparator());
          data.Type = vParts[0];
  ...
  static getDataAllOtherSpeed(charID) {
    return CharDataMapping.getDataAll(charID, "otherSpeeds", null).then(
      (dataArray) => {
        for (let data of dataArray) {
          let vParts = data.value.split(getSeparator());
          data.Type = vParts[0];
