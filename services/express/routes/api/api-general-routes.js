...
function getModelByNameOrID(
  ...
  if (id != null) {
    if (hasHomebrewID) {
      return ModelType.findOne({ where: { id: id, homebrewID: null } }).then(
        (model) => {
          return model;
        },
      );
    } else {
      return ModelType.findOne({ where: { id: id } }).then((model) => {
        return model;
      });
    }
  } else {
    ...
    if (hasHomebrewID) {
      if (nameLike) {
        ...
        return ModelType.findOne({
          where: { name: { [Op.like]: name }, homebrewID: null },
        }).then((model) => {
          return model;
        });
      } else {
        return ModelType.findOne({
          where: { name: name, homebrewID: null },
        }).then((model) => {
          return model;
        });
      }
      ...
      if (nameLike) {
        name = `%${name.replace(/ /g, "% %").trim()}%`;
        return ModelType.findOne({ where: { name: { [Op.like]: name } } }).then(
          (model) => {
            return model;
          },
        );
      } else {
        return ModelType.findOne({ where: { name: name } }).then((model) => {
          return model;
        });
      }
    }
...
router.get("/char", (req, res) => {
  if (req.query.name != null || req.query.id != null) {
    getModelByNameOrID(Character, req.query.name, req.query.id, false).then(
      (character) => {
        if (character != null) {
...
router.get("/item", (req, res) => {
  if (req.query.name != null || req.query.id != null) {
    getModelByNameOrID(Item, req.query.name, req.query.id, true, true).then(
      (item) => {
        if (item != null) {
...
router.get("/ancestry", (req, res) => {
  if (req.query.name != null || req.query.id != null) {
    getModelByNameOrID(Ancestry, req.query.name, req.query.id).then(
      (ancestry) => {
        if (ancestry != null) {
...
router.get("/archetype", (req, res) => {
  if (req.query.name != null || req.query.id != null) {
    getModelByNameOrID(Archetype, req.query.name, req.query.id).then(
      (archetype) => {
        if (archetype != null) {
          GeneralGathering.getArchetype(-1, archetype.id).then(
            (archetypeData) => {
...
router.get("/v-heritage", (req, res) => {
  if (req.query.name != null || req.query.id != null) {
    getModelByNameOrID(UniHeritage, req.query.name, req.query.id).then(
      (uniHeritage) => {
        if (uniHeritage != null) {
          GeneralGathering.getUniHeritage(-1, uniHeritage.id).then(
            (uniHeritageData) => {
...
router.get("/heritage", (req, res) => {
  if (req.query.name != null || req.query.id != null) {
    getModelByNameOrID(Heritage, req.query.name, req.query.id).then(
      (heritage) => {
        if (heritage != null) {
...
router.get("/background", (req, res) => {
  if (req.query.name != null || req.query.id != null) {
    getModelByNameOrID(Background, req.query.name, req.query.id).then(
      (background) => {
        if (background != null) {
          GeneralGathering.getBackground(-1, background.id).then(
            (backgroundData) => {
...
router.get("/condition", (req, res) => {
  if (req.query.name != null || req.query.id != null) {
    getModelByNameOrID(Condition, req.query.name, req.query.id, false).then(
      (condition) => {
        if (condition != null) {
          GeneralGathering.getCondition(-1, condition.id).then(
            (conditionData) => {
