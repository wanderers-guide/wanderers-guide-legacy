...
module.exports = class GeneralGathering {
  ...
  static getCondition(userID, conditionID) {
    return Condition.findOne({ where: { id: conditionID } }).then(
      (condition) => {
        return {
          condition: condition,
        };
          ...
          return AncestryLanguage.findAll({
            ...
            return AncestryBoost.findAll({
              ...
              return AncestryFlaw.findAll({
                ...
                return SenseType.findAll().then((senseTypes) => {
                  return CharGathering.getAllPhysicalFeatures(userID).then(
                    (physicalFeatures) => {
                      ...
                      for (let physicalFeature of physicalFeatures) {
                        if (
                          physicalFeature.id === ancestry.physicalFeatureOneID
                        ) {
                          physicalFeatureOne = physicalFeature;
                        } else if (
                          physicalFeature.id === ancestry.physicalFeatureTwoID
                        ) {
                      ...
                      let ancestryStruct = {
                        ...
                        boosts: [],
                        flaws: [],
                        vision_sense: visionSense,
                        additional_sense: additionalSense,
                        physical_feature_one: physicalFeatureOne,
                        physical_feature_two: physicalFeatureTwo,
                      };
  ...
  static async getAllFeats(
    ...
    if (feats == null) {
      feats = await Prisma.feats.findMany({
        where: {
          ...
          NOT:
            homebrewID == null && cache ? undefined : { id: -1 /*No-Cache*/ },
        },
    ...
    for (const feat of feats) {
      if (
        TempUnpublishedBooks.getSourcesArray(userID).includes(feat.contentSrc)
      ) {
        continue;
      }
      let fTags = [];
  ...
  static getFeat(userID, featID, homebrewID = null) {
    return Feat.findOne({
      ...
      return FeatTag.findAll({ where: { featID: featID } }).then((featTags) => {
        let featTagPromises = [];
        for (const featTag of featTags) {
          let newPromise = Tag.findOne({ where: { id: featTag.tagID } });
          featTagPromises.push(newPromise);
        }
        return Promise.all(featTagPromises).then(function (tags) {
          return {
            feat: feat,
            traits: tags,
          };
  ...
  static async getAllSpells(
    ...
    for (const spell of spells) {
      if (
        TempUnpublishedBooks.getSourcesArray(userID).includes(spell.contentSrc)
      ) {
        continue;
      }
  ...
  static getSpell(userID, spellID, homebrewID = null) {
    return Spell.findOne({
      ...
      return TaggedSpell.findAll({ where: { spellID: spellID } }).then(
        (spellTags) => {
          ...
          return Promise.all(spellTagPromises).then(function (tags) {
            return {
              spell: spell,
              traits: tags,
            };
  ...
  static async getAllExtras(
    ...
    for (const extra of extras) {
      if (
        TempUnpublishedBooks.getSourcesArray(userID).includes(extra.contentSrc)
      ) {
        continue;
      }
      let eTags = [];
  ...
  static getExtra(userID, extraID, homebrewID = null) {
    return Extra.findOne({
      ...
      return TaggedExtra.findAll({ where: { extraID: extraID } }).then(
        (taggedExtras) => {
          ...
          return Promise.all(taggedExtraPromises).then(function (tags) {
            return {
              extra: extra,
              traits: tags,
            };
  ...
  static getAllConditions(userID) {
    return Condition.findAll().then((allConditions) => {
  ...
  static async getAllItems(
    ...
    for (const item of items) {
      if (
        TempUnpublishedBooks.getSourcesArray(userID).includes(item.contentSrc)
      ) {
        continue;
      }
  ...
  static getItem(userID, itemID, homebrewID = null) {
    return Item.findOne({
      ...
      return TaggedItem.findAll({ where: { itemID: itemID } }).then(
        (itemTags) => {
          let itemTagPromises = [];
          ...
          return Promise.all(itemTagPromises).then(function (tags) {
            return Weapon.findOne({ where: { itemID: itemID } }).then(
              (weaponData) => {
                return Armor.findOne({ where: { itemID: itemID } }).then(
                  (armorData) => {
                    return Storage.findOne({ where: { itemID: itemID } }).then(
                      (storageData) => {
                        return Shield.findOne({
                          where: { itemID: itemID },
                        }).then((shieldData) => {
                          return ItemRune.findOne({
                            where: { itemID: itemID },
                          }).then((runeData) => {
                            return {
                              item: item,
                              traits: tags,
                              data_weapon:
                                weaponData != null ? weaponData : undefined,
                              data_armor:
                                armorData != null ? armorData : undefined,
                              data_storage:
                                storageData != null ? storageData : undefined,
                              data_shield:
                                shieldData != null ? shieldData : undefined,
                              data_rune:
                                runeData != null ? runeData : undefined,
                            };
                          });
                        });
                      },
