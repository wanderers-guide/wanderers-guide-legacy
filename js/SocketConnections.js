...
module.exports = class SocketConnections {
  ...
  static sheetGeneral(io) {
    ...
    io.on("connection", function (socket) {
      ...
      */
      ...
      socket.on(
        "requestUpdateCalculatedStats",
        function (charID, calcStatsStruct) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSaving.updateCalculatedStats(charID, calcStatsStruct).then(
                (result) => {
      ...
      socket.on(
        "requestNotesFieldSave",
        function (charID, notesData, notesFieldControlShellID) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSaving.saveNoteField(
                charID,
                notesData,
                notesData.placeholderText,
                notesData.text,
              ).then((result) => {
      ...
      socket.on(
        "requestCurrentHitPointsSave",
        function (charID, currentHealth) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSaving.saveCurrentHitPoints(charID, currentHealth).then(
                (result) => {
                  // Return nothing
                },
              );
      ...
      socket.on(
        "requestCurrentStaminaPointsSave",
        function (charID, currentStamina) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSaving.saveCurrentStaminaPoints(charID, currentStamina).then(
                (result) => {
                  // Return nothing
                },
              );
      ...
      socket.on(
        "requestCurrentResolvePointsSave",
        function (charID, currentResolve) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSaving.saveCurrentResolvePoints(charID, currentResolve).then(
                (result) => {
                  // Return nothing
                },
              );
      ...
      socket.on("requestRollHistorySave", function (charID, rollHistoryJSON) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            CharSaving.saveRollHistory(charID, rollHistoryJSON).then(
              (result) => {
    ...
    io.on("connection", function (socket) {
      ...
      socket.on("requestInvUpdate", function (charID) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            CharGathering.getCharacter(userID, charID).then((character) => {
              CharGathering.getInventory(userID, character.inventoryID).then(
                (invStruct) => {
      ...
      socket.on(
        "requestAddPropertyRune",
        function (invItemID, propRuneID, propRuneSlot) {
          AuthCheck.canEditInvItem(userID, invItemID).then((canEditItem) => {
            if (canEditItem) {
              CharGathering.getInvIDFromInvItemID(userID, invItemID).then(
                (invID) => {
                  CharSaving.addPropRune(
                    invItemID,
                    propRuneID,
                    propRuneSlot,
                  ).then(() => {
                    CharGathering.getInventory(userID, invID).then(
                      (invStruct) => {
      ...
      socket.on(
        "requestRemovePropertyRune",
        function (invItemID, propRuneSlot) {
          AuthCheck.canEditInvItem(userID, invItemID).then((canEditItem) => {
            if (canEditItem) {
              CharGathering.getInvIDFromInvItemID(userID, invItemID).then(
                (invID) => {
      ...
      socket.on(
        "requestRemoveFundamentalRune",
        function (invItemID, fundRuneID) {
          AuthCheck.canEditInvItem(userID, invItemID).then((canEditItem) => {
            if (canEditItem) {
              CharGathering.getInvIDFromInvItemID(userID, invItemID).then(
                (invID) => {
      ...
      socket.on(
        "requestAddItemToInv",
        function (charID, invID, itemID, quantity) {
          AuthCheck.canEditInv(userID, invID).then((canEditInv) => {
            if (canEditInv) {
              CharSaving.addItemToInv(invID, itemID, quantity).then(
                (invItem) => {
                  CharGathering.getInventory(userID, invID).then(
                    (invStruct) => {
                      CharGathering.getItem(userID, charID, itemID).then(
                        (item) => {
                          socket.emit(
                            "returnAddItemToInv",
                            item,
      ...
      socket.on(
        "requestInvItemMoveBag",
        function (invItemID, bagInvItemID, isDropped) {
          AuthCheck.canEditInvItem(userID, invItemID).then((canEditItem) => {
            if (canEditItem) {
              CharGathering.getInvIDFromInvItemID(userID, invItemID).then(
                (invID) => {
      ...
      socket.on(
        "requestAddItemToBag",
        function (itemID, quantity, bagInvItemID) {
          AuthCheck.canEditInvItem(userID, bagInvItemID).then((canEditItem) => {
            if (canEditItem) {
              CharGathering.getInvIDFromInvItemID(userID, bagInvItemID).then(
                (invID) => {
                  CharSaving.addItemToInv(invID, itemID, quantity).then(
                    (invItem) => {
                      CharSaving.saveInvItemToNewBag(
      ...
      socket.on(
        "requestAddItemCustomizeToInv",
        function (charID, invID, itemID, updateValues) {
          AuthCheck.canEditInv(userID, invID).then((canEditInv) => {
            if (canEditInv) {
  ...
  static sheetConditions(io) {
    ...
    io.on("connection", function (socket) {
      ...
      socket.on(
        "requestConditionAdd",
        function (charID, conditionID, value, sourceText, parentID) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSaving.addCondition(
                charID,
                conditionID,
                value,
                sourceText,
                ...
                CharGathering.getAllCharConditions(userID, charID).then(
                  (conditionsObject) => {
                    socket.emit(
                      "returnUpdateConditionsMap",
                      conditionsObject,
                      true,
                    );
                  },
      ...
      socket.on("requestConditionRemove", function (charID, conditionID) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            CharSaving.removeCondition(charID, conditionID).then(
              (didRemove) => {
                CharGathering.getAllCharConditions(userID, charID).then(
                  (conditionsObject) => {
                    socket.emit(
  ...
  static sheetSpells(io) {
    ...
    io.on("connection", function (socket) {
      ...
      socket.on(
        "requestSpellAddToSpellBook",
        function (charID, spellSRC, spellID, spellLevel, spellType) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSpells.addToSpellBook(
                charID,
                spellSRC,
                spellID,
                spellLevel,
                ...
                CharSpells.getSpellBook(charID, spellSRC, false).then(
                  (spellBookStruct) => {
      ...
      socket.on(
        "requestSpellRemoveFromSpellBook",
        function (charID, spellSRC, spellID, spellLevel) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSpells.removeFromSpellBook(
                charID,
                spellSRC,
                spellID,
                spellLevel,
      ...
      socket.on(
        "requestSpellTypeUpdate",
        function (charID, spellBookSpellID, spellType) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSpells.changeSpellBookSpellType(
                charID,
                spellBookSpellID,
                spellType,
              ).then((result) => {
      ...
      socket.on("requestSpellSlotUpdate", function (charID, updateSlotObject) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            CharSpells.changeSpellSlot(charID, updateSlotObject).then(
              (result) => {
      ...
      socket.on(
        "requestInnateSpellCastingUpdate",
        function (innateSpell, timesCast) {
          if (innateSpell != null) {
            AuthCheck.canEditCharacter(userID, innateSpell.charID).then(
              (canEditChar) => {
                if (canEditChar) {
                  CharSaving.saveInnateSpellCastings(
                    innateSpell,
                    timesCast,
                  ).then((result) => {
      ...
      socket.on(
        "requestFocusSpellUpdate",
        function (charID, srcStruct, spellSRC, spellID) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharDataMapping.setData(
                charID,
                "focusSpell",
                srcStruct,
                spellSRC + "=" + spellID,
      ...
      socket.on(
        "requestFocusPointUpdate",
        function (charID, srcStruct, unused) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharDataMapping.setData(
                charID,
  ...
  static sheetCompanions(io) {
    ...
    io.on("connection", function (socket) {
      ...
      socket.on("requestAddAnimalCompanion", function (charID, animalCompID) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            CharSaving.addAnimalCompanion(charID, animalCompID).then(
              (charAnimalComp) => {
      ...
      socket.on(
        "requestUpdateAnimalCompanion",
        function (charID, charAnimalCompID, updateValues) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSaving.updateAnimalCompanion(
                charID,
                charAnimalCompID,
                updateValues,
              ).then(() => {
      ...
      socket.on(
        "requestRemoveAnimalCompanion",
        function (charID, charAnimalCompID) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
      ...
      socket.on(
        "requestAddSpecificFamiliar",
        function (charID, specificStruct) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSaving.addSpecificFamiliar(charID, specificStruct).then(
                (charFamiliar) => {
                  socket.emit("returnAddFamiliar", charFamiliar);
                },
              );
      ...
      socket.on(
        "requestUpdateFamiliar",
        function (charID, charFamiliarID, updateValues) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSaving.updateFamiliar(
                charID,
                charFamiliarID,
                updateValues,
              ).then(() => {
    ...
    io.on("connection", function (socket) {
      ...
      socket.on("requestCharacterDetails", function (charID) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            CharGathering.getCharacter(userID, charID).then((character) => {
              BuildsGathering.getBuildInfo(character.buildID).then(
                (buildInfo) => {
                  ClientAPI.getClientsWhoAccess(charID).then(
                    (clientsWithAccess) => {
                      CampaignGathering.getCampaign(charID).then((campaign) => {
                        UserHomebrew.getCollectedHomebrewBundles(userID).then(
                          (hBundles) => {
                            UserHomebrew.getIncompleteHomebrewBundles(
                              userID,
                            ).then((progessBundles) => {
                              socket.emit(
                                "returnCharacterDetails",
                                character,
                                buildInfo,
                                clientsWithAccess,
                                campaign,
      ...
      socket.on(
        "requestCharacterRemoveClientAccess",
        function (charID, clientID) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              ClientAPI.removeAccessToken(charID, clientID).then((result) => {
                ClientAPI.getClientsWhoAccess(charID).then(
                  (clientsWithAccess) => {
                    socket.emit(
                      "returnCharacterRemoveClientAccess",
                      clientsWithAccess,
                    );
                  },
                );
      ...
      socket.on(
        "requestCharacterOptionChange",
        function (charID, optionName, value) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSaving.saveCharacterOption(charID, optionName, value).then(
                (result) => {
                  if (optionName == "variantAncestryParagon") {
                    // Hardcoded - Clear ancestry feats for Ancestry Paragon variant
                    CharDataMapping.deleteDataBySourceAndType(
                      charID,
                      "chosenFeats",
                      "ancestry",
                    ).then((result) => {
      ...
      socket.on(
        "requestCharacterSourceChange",
        function (charID, sourceName, isAdd) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (isAdd) {
                CharContentSources.addSource(charID, sourceName).then(
                  (result) => {
                    socket.emit("returnCharacterSourceChange");
                  },
                );
              } else {
                CharContentSources.removeSource(charID, sourceName).then(
                  (result) => {
                    socket.emit("returnCharacterSourceChange");
                  },
                );
      ...
      socket.on(
        "requestCharacterSetSources",
        function (charID, contentSourceArray) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharContentSources.setSources(charID, contentSourceArray).then(
                (result) => {
      ...
      socket.on(
        "requestCharacterHomebrewChange",
        function (charID, homebrewID, isAdd) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (isAdd) {
                CharContentHomebrew.addHomebrewBundle(
                  userID,
                  charID,
                  homebrewID,
                ).then((result) => {
                ...
                CharContentHomebrew.removeHomebrewBundle(
                  charID,
                  homebrewID,
                ).then((result) => {
                  socket.emit("returnCharacterHomebrewChange");
      ...
      socket.on(
        "requestCharacterCustomCodeBlockChange",
        function (charID, code) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
    ...
    io.on("connection", function (socket) {
      ...
      socket.on("requestBuilderPageAncestry", function (charID) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            CharGathering.getCharacter(userID, charID).then((character) => {
              CharGathering.getAllAncestries(
                userID,
                character.enabledSources,
                character.enabledHomebrew,
                false,
              ).then((ancestriesObject) => {
                CharGathering.getAllUniHeritages(userID, charID).then(
                  (uniHeritageArray) => {
                    socket.emit(
                      "returnBuilderPageAncestry",
                      ancestriesObject,
                      uniHeritageArray,
                    );
                  },
            ...
            socket.emit(
              "returnErrorMessage",
              "Incorrect Auth - No access to this character.",
            );
          }
        });
      });
      ...
      socket.on(
        "requestHeritageChange",
        function (charID, heritageID, isUniversal) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSaving.saveHeritage(charID, heritageID, isUniversal).then(
                (result) => {
                  return CharTags.getTags(charID).then((charTagsArray) => {
                    socket.emit(
                      "returnHeritageChange",
                      heritageID,
                      isUniversal,
                      charTagsArray,
                    );
    ...
    io.on("connection", function (socket) {
      ...
      socket.on("requestBuilderPageBackground", function (charID) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            CharGathering.getCharacter(userID, charID).then((character) => {
              CharGathering.getAllBackgrounds(
                userID,
                character.enabledSources,
                character.enabledHomebrew,
              ).then((backgrounds) => {
    ...
    io.on("connection", function (socket) {
      ...
      socket.on("requestBuilderPageFinalize", function (charID) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            CharGathering.getCharacter(userID, charID).then((character) => {
              CharGathering.getAllUnselectedData(userID, charID).then(
                (unselectedDataArray) => {
                  socket.emit(
                    "returnBuilderPageFinalize",
                    character,
                    unselectedDataArray,
                  );
                },
      ...
      socket.on(
        "requestLangsAndTrainingsClear",
        function (charID, profSrcStruct, langSrcStruct, data) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharDataMapping.deleteData(
                charID,
                "languages",
                langSrcStruct,
              ).then((result) => {
      ...
      socket.on("requestCustomCodeBlockDataClear", function (charID) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            CharDataMapping.deleteDataBySourceType(charID, "custom-code").then(
              (result) => {
  ...
  static builderGeneral(io) {
    ...
    io.on("connection", function (socket) {
      ...
      */
      ...
      socket.on(
        "requestUnselectedDataChange",
        function (charID, srcStruct, unselectedData, deleteOnly = true) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (unselectedData == null) {
                if (deleteOnly) {
                  CharDataMapping.deleteDataOnly(
                    charID,
                    "unselectedData",
                    srcStruct,
                  ).then((result) => {
      ...
      socket.on("requestUnselectedDataClear", function (charID) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            CharDataMapping.deleteDataBySourceType(
              charID,
              "unselectedData",
            ).then((result) => {
      ...
      socket.on(
        "requestAbilityBonusChange",
        function (charID, srcStruct, abilityBonusStruct) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (abilityBonusStruct == null) {
                ...
                CharDataMappingExt.setDataAbilityBonus(
                  charID,
                  srcStruct,
                  abilityBonusStruct.Ability,
                  abilityBonusStruct.Bonus,
                ).then((result) => {
                  socket.emit("returnAbilityBonusChange");
                });
              }
      ...
      socket.on(
        "requestClassChoiceChange",
        function (charID, srcStruct, classChoiceStruct, deleteSelfData = true) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (classChoiceStruct == null) {
                ...
                CharDataMappingExt.setDataClassChoice(
                  charID,
                  srcStruct,
                  classChoiceStruct.SelectorID,
                  classChoiceStruct.OptionID,
                  deleteSelfData,
                ).then((result) => {
                  socket.emit(
                    "returnClassChoiceChange",
                    srcStruct,
                    classChoiceStruct.SelectorID,
                    classChoiceStruct.OptionID,
                  );
      ...
      socket.on(
        "requestProficiencyChange",
        function (charID, profChangePacket, profStruct) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              ...
              if (profStruct == null) {
                ...
                CharDataMappingExt.setDataProficiencies(
                  charID,
                  srcStruct,
                  profStruct.For,
                  profStruct.To,
                  profStruct.Prof,
                  profStruct.SourceName,
                ).then((result) => {
                  socket.emit("returnProficiencyChange", profChangePacket);
                });
              }
        ...
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            ...
            if (featChangePacket.featID == null) {
              CharDataMapping.deleteData(charID, "chosenFeats", srcStruct).then(
                (result) => {
      ...
      socket.on(
        ...
        function (charID, srcStruct, langID, deleteSelfData = true) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (langID == null) {
                CharDataMapping.deleteData(charID, "languages", srcStruct).then(
                  (result) => {
      ...
      socket.on(
        "requestSensesChange",
        function (charID, srcStruct, senseID, deleteSelfData = true) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (senseID == null) {
                CharDataMapping.deleteData(charID, "senses", srcStruct).then(
                  (result) => {
      ...
      socket.on(
        "requestPhysicalFeaturesChange",
        function (charID, srcStruct, physicalFeatureID, deleteSelfData = true) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (physicalFeatureID == null) {
                CharDataMapping.deleteData(charID, "phyFeats", srcStruct).then(
                  (result) => {
        ...
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            if (domain == null || domain.Domain == null) {
              CharDataMapping.deleteData(charID, "domains", srcStruct).then(
                (result) => {
      ...
      socket.on(
        "requestDomainAdvancementChange",
        function (charID, srcStruct, domain) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (domain == null || domain.Domain == null) {
                CharDataMapping.deleteData(
  ...
  static builderGeneralProcessing(io) {
    ...
    io.on("connection", function (socket) {
      ...
      socket.on(
        "requestAddClassFeature",
        function (charID, srcStruct, featureName, inputPacket) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (featureName == null) {
                CharDataMapping.deleteData(
                ...
                CharGathering.getClassFeatureByName(
                  userID,
                  charID,
                  featureName,
                ).then((classFeature) => {
                  if (classFeature != null) {
                    CharDataMapping.setData(
                      ...
                      "classAbilityExtra",
                      srcStruct,
                      classFeature.id + ":::" + inputPacket.dontRunCode,
                    ).then((result) => {
                      if (classFeature.selectType == "SELECTOR") {
                        CharGathering.getCharacter(userID, charID).then(
                          (character) => {
                            CharGathering.getAllClassFeatureOptions(
                              userID,
                              character.enabledSources,
                              character.enabledHomebrew,
                            ).then((allOptions) => {
                    ...
                    socket.emit(
                      "returnWSCStatementFailure",
                      'Unknown Class Feature "' + featureName + '"',
                    );
                  }
                });
              }
      ...
      socket.on(
        "requestClassArchetypeChange",
        function (charID, srcStruct, classArchetypeID) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (classArchetypeID == null) {
                CharDataMapping.deleteData(
                  charID,
                  "classArchetypeChoice",
                  srcStruct,
                ).then((result) => {
      ...
      socket.on(
        "requestSCFSChange",
        function (charID, srcStruct, classFeatureID, inputPacket) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (classFeatureID == null) {
                CharDataMapping.deleteData(charID, "scfs", srcStruct).then(
                  (result) => {
      ...
      socket.on(
        "requestFindClassFeatureForSCFS",
        function (charID, featureName, inputPacket) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharGathering.getClassFeatureByName(
                userID,
                ...
                if (classFeature != null) {
                  if (classFeature.selectType == "SELECTOR") {
                    ...
                    socket.emit(
                      "returnWSCStatementFailure",
                      "Class Feature must have selection options for a SCFS!",
                    );
                  }
                } else {
                  socket.emit(
      ...
      socket.on(
        "requestAddHeritageEffect",
        function (charID, srcStruct, heritageName, inputPacket) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharGathering.getHeritageByName(
                userID,
                charID,
                heritageName,
              ).then((heritage) => {
      ...
      socket.on(
        "requestHeritageEffectsChange",
        function (charID, srcStruct, heritageID, deleteSelfData = true) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (heritageID == null) {
                CharDataMapping.deleteData(
      ...
      socket.on(
        "requestFindHeritagesFromAncestryName",
        function (charID, srcStruct, ancestryName, inputPacket) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharGathering.getHeritagesByAncestryName(
                userID,
                charID,
                ancestryName,
              ).then((heritages) => {
                if (heritages != null) {
                  socket.emit(
                    "returnFindHeritagesFromAncestryName",
                    srcStruct,
                    heritages,
                    inputPacket,
                  );
      ...
      socket.on(
        "requestWeaponFamiliarityChange",
        function (charID, srcStruct, trait) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharDataMapping.setData(
                charID,
      ...
      socket.on(
        "requestWeaponSpecializationChange",
        function (charID, srcStruct, type) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (type === 1 || type === 2 || type === 3) {
                ...
                socket.emit(
                  "returnWSCStatementFailure",
                  'Invalid weapon specialization type "' + type + '"',
                );
              }
            } else {
              socket.emit("returnWSCStatementFailure", "Incorrect Auth");
      ...
      socket.on(
        "requestArmorSpecializationChange",
        function (charID, srcStruct, armorName) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharDataMapping.setData(
                charID,
      ...
      socket.on(
        "requestWeaponCriticalSpecializationChange",
        function (charID, srcStruct, weapName) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharDataMapping.setData(
                charID,
      ...
      socket.on(
        "requestSpeedChange",
        function (charID, srcStruct, speedType, speedAmt) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              let amount = parseInt(speedAmt);
              if (!isNaN(amount) || speedAmt === "LAND_SPEED") {
                CharDataMappingExt.setDataOtherSpeed(
                  charID,
                  srcStruct,
                  speedType,
                  speedAmt,
      ...
      socket.on("requestCharTagChange", function (charID, srcStruct, charTag) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            CharTags.setTag(charID, srcStruct, charTag).then((result) => {
              return CharTags.getTags(charID).then((charTagsArray) => {
      ...
      socket.on("requestFeatChangeByID", function (charID, featChangePacket) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            if (
              featChangePacket.feat != null &&
              featChangePacket.feat.Feat != null
            ) {
              let srcStruct = featChangePacket.srcStruct;
              CharDataMapping.setDataOnly(
                charID,
              ...
              socket.emit(
                "returnWSCStatementFailure",
                "Cannot find feat passed to FeatChangeByID",
              );
            }
          } else {
            socket.emit("returnWSCStatementFailure", "Incorrect Auth");
      ...
      socket.on(
        "requestLanguageChangeByID",
        function (charID, srcStruct, langID) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharDataMapping.setDataOnly(
                charID,
      ...
      socket.on(
        "requestSensesChangeByID",
        function (charID, srcStruct, senseID) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharDataMapping.setDataOnly(
                charID,
      ...
      socket.on(
        "requestPhysicalFeaturesChangeByID",
        function (charID, srcStruct, phyFeatID) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharDataMapping.setDataOnly(
                charID,
      ...
      // Give Spell Slots //
      socket.on(
        "requestSpellCastingSlotChange",
        function (
          charID,
          srcStruct,
          spellSRC,
          spellcasting,
          reduceSlotsByOne = false,
        ) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSpells.setSpellCasting(
                charID,
                srcStruct,
                spellSRC,
                spellcasting,
                ...
                if (spellSlots != null) {
                  ...
                  socket.emit(
                    "returnWSCStatementFailure",
                    'Invalid Spellcasting "' + spellcasting + '"',
                  );
                }
              });
            } else {
      ...
      socket.on(
        "requestSpellSlotChange",
        function (charID, srcStruct, spellSRC, spellLevel, slotType = "") {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (spellLevel != null) {
                CharSpells.setSpellSlot(
                  charID,
                  srcStruct,
                  spellSRC,
                  spellLevel,
      ...
      socket.on(
        "requestKeySpellAbilityChange",
        function (charID, srcStruct, spellSRC, abilityScore) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharDataMapping.setData(
                charID,
      ...
      socket.on(
        "requestSpellCastingTypeChange",
        function (charID, srcStruct, spellSRC, castingType) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharDataMapping.setData(
                charID,
      ...
      socket.on(
        "requestSpellTraditionChange",
        function (charID, srcStruct, spellSRC, spellList) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (
                spellList == "OCCULT" ||
                ...
                socket.emit(
                  "returnWSCStatementFailure",
                  'Invalid Spell Tradition "' + spellList + '"',
                );
              }
            } else {
              socket.emit("returnWSCStatementFailure", "Incorrect Auth");
      ...
      socket.on(
        "requestBuilderSpellAddToSpellBook",
        function (
          charID,
          srcStruct,
          spellSRC,
          ...
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              ...
              if (!isNaN(sLevel)) {
                CharGathering.getSpellByName(userID, charID, spellName).then(
                  (spell) => {
                    if (spell != null) {
                      if (spell.level <= sLevel) {
                        CharSpells.addToSpellBookFromBuilder(
                          charID,
                          spellSRC,
                          spell.id,
                          sLevel,
                        ...
                        socket.emit(
                          "returnWSCStatementFailure",
                          "Spell level cannot be lower than minimum spell level!",
                        );
                      }
                    } else {
                      socket.emit(
                        "returnWSCStatementFailure",
                        'Invalid Spell "' + spellName + '"',
                      );
                    }
                  },
                );
          ...
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (
                ...
                if (!isNaN(tPd) && !isNaN(sLevel)) {
                  CharGathering.getSpellByName(userID, charID, spellName).then(
                    (spell) => {
                      if (spell != null) {
                        if (spell.level <= sLevel) {
                          CharDataMappingExt.setDataInnateSpell(
                            charID,
                            srcStruct,
                            spell.id,
                            sLevel,
                ...
                socket.emit(
                  "returnWSCStatementFailure",
                  'Invalid Spell Tradition "' + spellTradition + '"',
                );
              }
            } else {
              socket.emit("returnWSCStatementFailure", "Incorrect Auth");
      ...
      // Focus Spell //
      socket.on(
        "requestFocusSpellChange",
        function (charID, srcStruct, spellSRC, spellName) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharGathering.getSpellByName(userID, charID, spellName).then(
                (spell) => {
      ...
      socket.on("requestFocusPointChange", function (charID, srcStruct) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            CharDataMapping.setData(charID, "focusPoint", srcStruct, "1").then(
              (result) => {
      ...
      // Resistances //
      socket.on(
        "requestResistanceChange",
        function (charID, srcStruct, resistType, resistAmount) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (resistType == null || resistAmount == null) {
                ...
                CharDataMappingExt.setDataResistance(
                  charID,
                  srcStruct,
                  resistType,
                  resistAmount,
      ...
      socket.on(
        "requestVulnerabilityChange",
        function (charID, srcStruct, vulnerType, vulnerAmount) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (vulnerType == null || vulnerAmount == null) {
                ...
                CharDataMappingExt.setDataVulnerability(
                  charID,
                  srcStruct,
                  vulnerType,
                  vulnerAmount,
      ...
      socket.on(
        "requestNotesFieldChange",
        function (charID, srcStruct, placeholderText, noteChangePacket) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharDataMapping.getDataSingle(
                charID,
                "notesField",
                srcStruct,
              ).then((notesData) => {
                if (notesData == null) {
                  CharSaving.saveNoteField(
                    ...
                    socket.emit(
                      "returnNotesFieldChange",
                      newNotesData,
                      noteChangePacket,
                    );
                  });
                } else {
                  CharGathering.getNoteField(userID, charID, notesData).then(
                    (newNotesData) => {
                      socket.emit(
                        "returnNotesFieldChange",
                        newNotesData,
      ...
      socket.on("requestNotesFieldDelete", function (charID, srcStruct) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            CharDataMapping.deleteData(charID, "notesField", srcStruct).then(
              (result) => {
  ...
  static builderWSC(io) {
    ...
    io.on("connection", function (socket) {
      ...
      socket.on("requestDataClearAtSrcStruct", function (charID, srcStruct) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            CharDataMapping.deleteDataBySourceStruct(charID, srcStruct).then(
              (result) => {
      ...
      socket.on(
        "requestMetaDataSetOnly",
        function (charID, source, srcStruct, value) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharDataMapping.setDataOnly(
                charID,
      ...
      */
      ...
      socket.on("requestWSCMapsInit", function (charID) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            CharGathering.getCharacter(userID, charID).then((character) => {
              CharGathering.getAllFeats(
                userID,
                character.enabledSources,
                character.enabledHomebrew,
              ).then((featsObject) => {
                CharGathering.getAllLanguages(userID, charID).then(
                  (langsObject) => {
                    CharGathering.getAllSpells(
                      userID,
                      character.enabledSources,
                      character.enabledHomebrew,
                    ).then((spellMap) => {
                      CharGathering.getAllArchetypes(userID, charID).then(
                        (archetypesArray) => {
                          socket.emit("returnWSCUpdateFeats", featsObject);
                          socket.emit("returnWSCUpdateLangs", langsObject);
                          socket.emit(
                            "returnWSCUpdateSpells",
                            mapToObj(spellMap),
                          );
                          socket.emit(
                            "returnWSCUpdateArchetypes",
                            archetypesArray,
                          );
        ...
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            if (updateType == "ABILITY-BOOSTS") {
              CharGathering.getChoicesAbilityBonus(userID, charID).then(
                (bonusDataArray) => {
                  socket.emit(
                    "returnWSCUpdateChoices",
                    "ABILITY-BOOSTS",
                    bonusDataArray,
                  );
                },
              ...
              CharGathering.getChoicesFeats(userID, charID).then(
                (featDataArray) => {
                  socket.emit("returnWSCUpdateChoices", "FEATS", featDataArray);
                },
              );
            } else if (updateType == "DOMAINS") {
              CharGathering.getChoicesDomains(userID, charID).then(
                (domainDataArray) => {
                  socket.emit(
                    "returnWSCUpdateChoices",
                    "DOMAINS",
      ...
      socket.on(
        "requestWSCInnateSpellChange",
        function (
          charID,
          srcStruct,
          innateSpellStruct,
          selectControlShellClass,
        ) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              if (innateSpellStruct == null) {
                CharDataMapping.deleteData(
                  ...
                  socket.emit(
                    "returnWSCInnateSpellChange",
                    selectControlShellClass,
                  );
                });
              } else {
                CharGathering.getSpellByName(
                  userID,
                  charID,
                  innateSpellStruct.name,
                ).then((spell) => {
                  if (spell != null) {
                    CharDataMappingExt.setDataInnateSpell(
                      charID,
                      srcStruct,
                      spell.id,
                      innateSpellStruct.level,
                      innateSpellStruct.tradition,
                      innateSpellStruct.tPd,
                    ).then((result) => {
                      socket.emit(
                        "returnWSCInnateSpellChange",
                        selectControlShellClass,
      ...
      socket.on("requestWSCSrcStructDataClear", function (charID, srcStruct) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            CharDataMapping.deleteDataSNumChildren(charID, srcStruct).then(
              (result) => {
  ...
  static charDataManagement(io) {
    ...
    io.on("connection", function (socket) {
      ...
      socket.on("requestCharImport", function (charExportData) {
        CharImport.importData(socket, charExportData).then((result) => {
      ...
      socket.on("requestCharExport", function (charID) {
        AuthCheck.canViewCharacter(userID, charID).then((canViewChar) => {
          if (canViewChar) {
            CharExport.getExportData(userID, charID).then((charExportData) => {
            ...
            CharExport.getExportData(userID, charID).then((charExportData) => {
              CharGathering.getCharacter(userID, charID).then((character) => {
                CharGathering.getAllFeats(
                  ...
                  socket.emit("returnCharExportPDFInfo", charExportData, {
                    featsObject,
                  });
                });
              });
  ...
  static homePage(io) {
    ...
    io.on("connection", function (socket) {
      ...
      socket.on(
        "requestBackgroundReport",
        function (backgroundID, email, message) {
      ...
      socket.on("requestProfileNameChange", function (newName) {
        if (GeneralUtils.validateProfileName(newName)) {
          if (userID && userID != -1) {
            ...
            User.update(updateValues, { where: { id: userID } }).then(
              (result) => {
  ...
  static adminPanel(io) {
    ...
    io.on("connection", function (socket) {
        ...
        AuthCheck.isAdmin(userID).then((isAdmin) => {
          if (isAdmin) {
            if (data != null && data.ancestryID != null) {
              AdminCreation.archiveAncestry(data.ancestryID, true).then(
                (result) => {
                  AdminCreation.addAncestry(data).then((result) => {
                    socket.emit("returnAdminCompleteAncestry");
                  });
      ...
      socket.on("requestAdminAncestryDetails", function () {
        AuthCheck.isAdmin(userID).then((isAdmin) => {
          if (isAdmin) {
            GeneralGathering.getAllAncestries(userID, true).then(
              (ancestriesObject) => {
                GeneralGathering.getAllFeats(
                  userID,
                  null,
                  null,
                  null,
      ...
      socket.on("requestAdminFeatDetails", function () {
        AuthCheck.isAdmin(userID).then((isAdmin) => {
          if (isAdmin) {
            GeneralGathering.getAllFeats(userID, null, null, null, false).then(
              (featsObject) => {
            ...
            GeneralGathering.getAllFeats(userID, null, null, null, false).then(
              (featsObject) => {
                GeneralGathering.getAllClasses(userID).then((classObject) => {
                  GeneralGathering.getAllAncestries(userID, true).then(
                    (ancestriesObject) => {
                      GeneralGathering.getAllUniHeritages(userID).then(
                        (uniHeritageArray) => {
                          GeneralGathering.getAllArchetypes(userID).then(
                            (archetypeArray) => {
                              socket.emit(
                                "returnAdminFeatDetailsPlus",
                                featsObject,
                                classObject,
                                ancestriesObject,
                                uniHeritageArray,
      ...
      socket.on("requestAdminItemDetails", function () {
        AuthCheck.isAdmin(userID).then((isAdmin) => {
          if (isAdmin) {
            GeneralGathering.getAllItems(userID, null, null, null, false).then(
              (itemMap) => {
        ...
        AuthCheck.isAdmin(userID).then((isAdmin) => {
          if (isAdmin) {
            if (data != null && data.backgroundID != null) {
              AdminCreation.archiveBackground(data.backgroundID, true).then(
                (result) => {
                  AdminCreation.addBackground(data).then((result) => {
                    socket.emit("returnAdminCompleteBackground");
                  });
        ...
        AuthCheck.isAdmin(userID).then((isAdmin) => {
          if (isAdmin) {
            if (data != null && data.archetypeID != null) {
              AdminCreation.archiveArchetype(data.archetypeID, true).then(
                (result) => {
                  AdminCreation.addArchetype(data).then((result) => {
                    socket.emit("returnAdminCompleteArchetype");
                  });
        ...
        AuthCheck.isAdmin(userID).then((isAdmin) => {
          if (isAdmin) {
            if (data != null && data.uniHeritageID != null) {
              AdminCreation.archiveUniHeritage(data.uniHeritageID, true).then(
                (result) => {
                  AdminCreation.addUniHeritage(data).then((result) => {
                    socket.emit("returnAdminCompleteUniHeritage");
                  });
        ...
        AuthCheck.isAdmin(userID).then((isAdmin) => {
          if (isAdmin) {
            if (data != null && data.heritageID != null) {
              AdminCreation.archiveHeritage(data.heritageID, true).then(
                (result) => {
                  AdminCreation.addHeritage(null, data).then((result) => {
                    socket.emit("returnAdminCompleteHeritage");
                  });
        ...
        AuthCheck.isAdmin(userID).then((isAdmin) => {
          if (isAdmin) {
            GeneralGathering.getAllHeritages(userID).then((heritageArray) => {
              GeneralGathering.getAllAncestriesBasic(userID).then(
                (ancestryArray) => {
                  socket.emit(
                    "returnAdminHeritageDetails",
                    heritageArray,
                    ancestryArray,
                  );
                },
        ...
        AuthCheck.isAdmin(userID).then((isAdmin) => {
          if (isAdmin) {
            if (data != null && data.classFeatureID != null) {
              AdminCreation.archiveClassFeature(data.classFeatureID, true).then(
                (result) => {
                  AdminCreation.addClassFeature(data).then((result) => {
                    socket.emit("returnAdminCompleteClassFeature");
                  });
      ...
      socket.on("requestAdminExtraDetails", function () {
        AuthCheck.isAdmin(userID).then((isAdmin) => {
          if (isAdmin) {
            GeneralGathering.getAllExtras(userID, null, null, null, false).then(
              (extraObj) => {
  ...
  static appAPI(io) {
    ...
    io.on("connection", function (socket) {
      ...
      // Request Access //
      socket.on(
        "requestAPIRequestAccess",
        function (clientID, charID, accessRights) {
          AuthCheck.ownsCharacter(userID, charID).then((ownsChar) => {
            if (ownsChar) {
              ClientAPI.getRequestAccessData(
                clientID,
                charID,
                accessRights,
              ).then((accessData) => {
      ...
      */
      ...
      socket.on("requestGeneralClass", function (classID, homebrewID = null) {
        if (homebrewID == null) {
          ...
          UserHomebrew.canAccessHomebrewBundle(userID, homebrewID).then(
            (canAccess) => {
              if (canAccess) {
                GeneralGathering.getClass(userID, classID, homebrewID).then(
                  (classStruct) => {
                    socket.emit("returnGeneralClass", classStruct);
                  },
                );
      ...
      socket.on(
        "requestGeneralAncestry",
        function (ancestryID, homebrewID = null) {
          if (homebrewID == null) {
            GeneralGathering.getAncestry(userID, ancestryID).then(
              (ancestryStruct) => {
      ...
      socket.on(
        "requestGeneralArchetype",
        function (archetypeID, homebrewID = null) {
          if (homebrewID == null) {
            GeneralGathering.getArchetype(userID, archetypeID).then(
              (archetypeStruct) => {
            ...
            UserHomebrew.canAccessHomebrewBundle(userID, homebrewID).then(
              (canAccess) => {
                if (canAccess) {
                  GeneralGathering.getArchetype(
                    userID,
                    archetypeID,
                    homebrewID,
                  ).then((archetypeStruct) => {
                    socket.emit("returnGeneralArchetype", archetypeStruct);
                  });
                }
              },
      ...
      socket.on(
        "requestGeneralBackground",
        function (backgroundID, homebrewID = null) {
          if (homebrewID == null) {
            GeneralGathering.getBackground(userID, backgroundID).then(
              (backgroundStruct) => {
            ...
            UserHomebrew.canAccessHomebrewBundle(userID, homebrewID).then(
              (canAccess) => {
                if (canAccess) {
                  GeneralGathering.getBackground(
                    userID,
                    backgroundID,
                    homebrewID,
                  ).then((backgroundStruct) => {
                    socket.emit("returnGeneralBackground", backgroundStruct);
                  });
                }
              },
      ...
      socket.on(
        "requestGeneralUniHeritage",
        function (uniHeritageID, homebrewID = null) {
          if (homebrewID == null) {
            GeneralGathering.getUniHeritage(userID, uniHeritageID).then(
              (uniHeritageStruct) => {
            ...
            UserHomebrew.canAccessHomebrewBundle(userID, homebrewID).then(
              (canAccess) => {
                if (canAccess) {
                  GeneralGathering.getUniHeritage(
                    userID,
                    uniHeritageID,
                    homebrewID,
                  ).then((uniHeritageStruct) => {
                    socket.emit("returnGeneralUniHeritage", uniHeritageStruct);
                  });
                }
              },
  ...
  static gmTools(io) {
    ...
    io.on("connection", function (socket) {
      ...
      socket.on("requestShopGeneratorDetails", function () {
        ...
        CharContentHomebrew.getEnabledHomebrewForCollection(userID).then(
          (homebrewStruct) => {
            CharContentSources.getEnabledSourcesForAllBooks(userID).then(
              (sourcesStruct) => {
                AuthCheck.isSupporter(userID).then((isSupporter) => {
                  ...
                  CharGathering.getAllItems(
                    userID,
                    sourcesStruct.enabledSources,
                    homebrewStruct.enabledHomebrew,
                  ).then((itemMap) => {
                    CharGathering.getAllTags(
                      userID,
                      homebrewStruct.enabledHomebrew,
                    ).then((traits) => {
                      socket.emit(
                        "returnShopGeneratorDetails",
                        mapToObj(itemMap),
                        traits,
                        isSupporter,
                        sourcesStruct,
      ...
      socket.on("requestEncounterDetails", function () {
        GeneralGathering.getAllCreatures(userID).then((allCreatures) => {
            ...
            GeneralGathering.getAllFeats(userID).then((featsObject) => {
              GeneralGathering.getAllItems(userID).then((itemMap) => {
                GeneralGathering.getAllSpells(userID).then((spellMap) => {
                  GeneralGathering.getAllConditions(userID).then(
                    (allConditions) => {
                      GeneralGathering.getAllSkills(null).then(
                        (skillObject) => {
                          CampaignGathering.getOwnedCampaigns(userID).then(
                            (campaigns) => {
                              socket.emit(
                                "returnEncounterDetails",
                                allCreatures,
                                allTags,
                                featsObject,
                                mapToObj(itemMap),
      ...
      socket.on("requestCampaignDetails", function (campaignID) {
        AuthCheck.ownsCampaign(userID, campaignID).then((ownsCampaign) => {
          if (ownsCampaign) {
            CampaignGathering.getCampaignDetails(campaignID).then(
              (campaignDetails) => {
                GeneralGathering.getAllConditions(userID).then(
                  (allConditions) => {
                    socket.emit(
                      "returnCampaignDetails",
                      campaignDetails,
                      allConditions,
                    );
                  },
      ...
      socket.on("requestEditCampaign", function (campaignID, updates) {
        AuthCheck.ownsCampaign(userID, campaignID).then((ownsCampaign) => {
          if (ownsCampaign) {
            CampaignSaving.updateCampaign(campaignID, updates).then(
              (result) => {
      ...
      socket.on("requestJoinCampaign", function (charID, accessCode) {
        AuthCheck.ownsCharacter(userID, charID).then((ownsChar) => {
          if (ownsChar) {
            CampaignSaving.joinCampaign(userID, charID, accessCode).then(
              (campaign) => {
  ...
  static remoteUpdate(io) {
    ...
    io.on("connection", function (socket) {
            ...
            CampaignGathering.getCampaign(charID).then((campaign) => {
              if (campaign) {
                ...
                if (campaign.optionDisplayPlayerHealth != 0) {
                  CampaignGathering.getUsersInCampaign(campaign.id).then(
                    (userIDs) => {
                      for (let uID of userIDs) {
                        socket
                          .to(uID)
                          .emit("sendCharacterUpdateToGM", charID, updates);
                      }
                    },
                  );
                }
      ...
      socket.on("requestCharacterUpdate-Health", function (charID, newHP) {
        AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
          if (canEditChar) {
            ...
            CampaignGathering.getUsersInCampaignFromChar(charID).then(
              (userIDs) => {
                for (let uID of userIDs) {
                  socket
                    .to(uID)
                    .emit("sendCharacterUpdate-Health", charID, newHP);
                }
              },
            );
          }
      ...
      socket.on(
        "requestCharacterUpdate-TempHealth",
        function (charID, newTempHP) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
                ...
                (userIDs) => {
                  for (let uID of userIDs) {
                    socket
                      .to(uID)
                      .emit(
                        "sendCharacterUpdate-TempHealth",
                        charID,
                        newTempHP,
                      );
                  }
            ...
            CampaignGathering.getUsersInCampaignFromChar(charID).then(
              (userIDs) => {
                for (let uID of userIDs) {
                  socket
                    .to(uID)
                    .emit("sendCharacterUpdate-Exp", charID, newExp);
                }
              },
            );
          }
      ...
      socket.on(
        "requestCharacterUpdate-Stamina",
        function (charID, newStamina) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSaving.saveCurrentStaminaPoints(charID, newStamina).then(
                (result) => {
                  // Return nothing
                },
              );
              ...
              CampaignGathering.getUsersInCampaignFromChar(charID).then(
                (userIDs) => {
                  for (let uID of userIDs) {
                    socket
                      .to(uID)
                      .emit("sendCharacterUpdate-Stamina", charID, newStamina);
                  }
                },
              );
            }
      ...
      socket.on(
        "requestCharacterUpdate-Resolve",
        function (charID, newResolve) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSaving.saveCurrentResolvePoints(charID, newResolve).then(
                (result) => {
                  // Return nothing
                },
              );
              ...
              CampaignGathering.getUsersInCampaignFromChar(charID).then(
                (userIDs) => {
                  for (let uID of userIDs) {
                    socket
                      .to(uID)
                      .emit("sendCharacterUpdate-Resolve", charID, newResolve);
                  }
                },
              );
            }
      ...
      socket.on(
        "requestCharacterUpdate-HeroPoints",
        function (charID, heroPoints) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSaving.saveHeroPoints(charID, heroPoints).then((result) => {
                // Return nothing
                ...
                (userIDs) => {
                  for (let uID of userIDs) {
                    socket
                      .to(uID)
                      .emit(
                        "sendCharacterUpdate-HeroPoints",
                        charID,
                        heroPoints,
                      );
                  }
      ...
      socket.on(
        "requestCharacterUpdate-ConditionAdd",
        function (
          charID,
          conditionName,
          conditionID,
          ...
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSaving.addCondition(
                ...
                CharSaving.updateCalculatedStats(charID, calculatedStats).then(
                  (result) => {
                    // Return nothing
                ...
                CharGathering.getAllCharConditions(null, charID).then(
                  (conditionsObject) => {
                    CampaignGathering.getUsersInCampaignFromChar(charID).then(
                      (userIDs) => {
                        for (let uID of userIDs) {
                          socket
                            .to(uID)
                            .emit(
                              "sendCharacterUpdate-Conditions",
                              charID,
                              conditionsObject,
                              true,
                            );
      ...
      socket.on(
        "requestCharacterUpdate-ConditionUpdate",
        function (
          charID,
          conditionID,
          value,
      ...
      socket.on(
        "requestCharacterUpdate-ConditionRemove",
        function (charID, conditionID, calculatedStats) {
          AuthCheck.canEditCharacter(userID, charID).then((canEditChar) => {
            if (canEditChar) {
              CharSaving.removeCondition(charID, conditionID).then(
                (didRemove) => {
                  CharSaving.updateCalculatedStats(
                    charID,
                    calculatedStats,
                  ).then((result) => {
                    // Return nothing
      ...
      */
        ...
        User.findOne({ where: { id: userID } }).then((user) => {
          if (user != null) {
            BuildsGathering.getBuild(buildID).then((build) => {
              CharSaving.createNewCharacter(user, build).then((character) => {
                if (character != null) {
      ...
      socket.on("requestBuildContents", function (buildID) {
        AuthCheck.canViewBuild(userID, buildID).then((viewBuild) => {
          if (viewBuild) {
            BuildsGathering.getBuildContents(userID, buildID).then(
              (buildContents) => {
      ...
      socket.on("requestBuildInfo", function (buildID) {
        BuildsGathering.getBasicBuildInfo(userID, buildID).then((build) => {
          UserHomebrew.getCollectedHomebrewBundles(userID).then((hBundles) => {
            UserHomebrew.getIncompleteHomebrewBundles(userID).then(
              (progessBundles) => {
      ...
      socket.on(
        "requestBuildDescriptionChange",
        function (buildID, description) {
          AuthCheck.canEditBuild(userID, buildID).then((editBuild) => {
            if (editBuild) {
              BuildsSaving.saveDescription(buildID, description).then(
                (result) => {
      ...
      socket.on(
        "requestBuildContactInfoChange",
        function (buildID, contactInfo) {
          AuthCheck.canEditBuild(userID, buildID).then((editBuild) => {
            if (editBuild) {
              BuildsSaving.saveContactInfo(buildID, contactInfo).then(
                (result) => {
      ...
      socket.on(
        "requestBuildOptionChange",
        function (buildID, optionName, value) {
          AuthCheck.canEditBuild(userID, buildID).then((editBuild) => {
            if (editBuild) {
              BuildsSaving.saveOption(buildID, optionName, value).then(
                (result) => {
                  if (
                    optionName == "variantAncestryParagon" ||
                    optionName == "variantGradualAbilityBoosts"
                  ) {
                    // Clear all meta data for Ancestry Paragon or Gradual Ability Boosts variant
                    BuildsSaving.clearMetaData(buildID).then((result) => {
      ...
      socket.on(
        "requestBuildSourceChange",
        function (buildID, sourceName, isAdd) {
          AuthCheck.canEditBuild(userID, buildID).then((editBuild) => {
            if (editBuild) {
              if (isAdd) {
                ...
                BuildsSaving.removeSource(buildID, sourceName).then(
                  (result) => {
                    socket.emit("returnBuildSourceChange");
                  },
                );
      ...
      socket.on(
        "requestBuildSetSources",
        function (buildID, contentSourceArray) {
          AuthCheck.canEditBuild(userID, buildID).then((editBuild) => {
            if (editBuild) {
              BuildsSaving.setSources(buildID, contentSourceArray).then(
                (result) => {
      ...
      socket.on(
        "requestBuildHomebrewChange",
        function (buildID, homebrewID, isAdd) {
          AuthCheck.canEditBuild(userID, buildID).then((editBuild) => {
            if (editBuild) {
              if (isAdd) {
                BuildsSaving.addHomebrewBundle(
                  userID,
                  buildID,
                  homebrewID,
                ).then((result) => {
                ...
                BuildsSaving.removeHomebrewBundle(buildID, homebrewID).then(
                  (result) => {
                    socket.emit("returnBuildHomebrewChange");
                  },
                );
      ...
      socket.on(
        "requestBuildUpdateMetaData",
        function (buildID, metaDataObject) {
          AuthCheck.canEditBuild(userID, buildID).then((editBuild) => {
            if (editBuild) {
              BuildsSaving.updateMetaData(buildID, metaDataObject).then(
                (result) => {
      ...
      socket.on(
        "requestBuildUpdateFinalStats",
        function (buildID, finalStatistics) {
          AuthCheck.canEditBuild(userID, buildID).then((editBuild) => {
            if (editBuild) {
              BuildsSaving.updateFinalStats(buildID, finalStatistics).then(
                (result) => {
  ...
  static homebrewGeneral(io) {
    ...
    io.on("connection", function (socket) {
      ...
      socket.on("requestHomebrewBundle", function (REQUEST_TYPE, homebrewID) {
        UserHomebrew.getHomebrewBundle(userID, homebrewID).then(
          (homebrewBundle) => {
      ...
      socket.on("requestHomebrewBundlesFromArray", function (homebrewArrayIDs) {
        UserHomebrew.getHomebrewBundlesFromArray(userID, homebrewArrayIDs).then(
          (hBundles) => {
      ...
      socket.on("requestBundleUpdate", function (homebrewID, updateValues) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            UserHomebrew.updateHomebrewBundle(
              userID,
              homebrewID,
              updateValues,
            ).then((homebrewBundle) => {
      ...
      socket.on(
        "requestBundleContents",
        function (REQUEST_TYPE, homebrewID, keyCode = "None") {
          UserHomebrew.getHomebrewBundle(userID, homebrewID).then(
            (homebrewBundle) => {
              UserHomebrew.hasHomebrewBundle(userID, homebrewID).then(
                (userHasBundle) => {
                  UserHomebrew.ownsHomebrewBundle(userID, homebrewID).then(
                    (userOwnsBundle) => {
                      UserHomebrew.canEditHomebrew(userID, homebrewID).then(
                        (canEdit) => {
                          if (REQUEST_TYPE == "EDIT" && !canEdit) {
                            return;
                          }
                          UserHomebrew.getValidKey(homebrewID, keyCode).then(
                            (bundleKey) => {
                              if (
                                REQUEST_TYPE == "VIEW" &&
                                homebrewBundle.hasKeys === 1 &&
                                !userHasBundle &&
                                bundleKey == null
                              ) {
                                socket.emit(
                                  "returnBundleContents",
                                  "REQUIRE-KEY",
                                  userHasBundle,
                                  userOwnsBundle,
                                  null,
                                ...
                                GeneralGathering.getAllSkills(null).then(
                                  (skillObject) => {
                                    HomebrewGathering.getAllClasses(
                                      homebrewID,
                                    ).then((classes) => {
                                      HomebrewGathering.getAllAncestries(
                                        homebrewID,
                                      ).then((ancestries) => {
                                        HomebrewGathering.getAllArchetypes(
                                          homebrewID,
                                        ).then((archetypes) => {
                                          HomebrewGathering.getAllBackgrounds(
                                            homebrewID,
                                          ).then((backgrounds) => {
                                            HomebrewGathering.getAllClassFeatures(
                                              homebrewID,
                                            ).then((classFeatures) => {
                                              HomebrewGathering.getAllFeats(
                                                homebrewID,
                                              ).then((feats) => {
                                                HomebrewGathering.getAllHeritages(
                                                  homebrewID,
                                                ).then((heritages) => {
                                                  HomebrewGathering.getAllUniHeritages(
                                                    homebrewID,
                                                  ).then((uniheritages) => {
                                                    HomebrewGathering.getAllItems(
                                                      homebrewID,
                                                    ).then((items) => {
                                                      HomebrewGathering.getAllSpells(
                                                        homebrewID,
                                                      ).then((spells) => {
                                                        GeneralGathering.getAllTags(
                                                          null,
                                                          homebrewID,
                                                        ).then((allTags) => {
                                                          HomebrewGathering.getAllLanguages(
                                                            homebrewID,
                                                          ).then(
                                                            (languages) => {
                                                              HomebrewGathering.getAllToggleables(
                                                                homebrewID,
                                                              ).then(
                                                                (
                                                                  toggleables,
      ...
      socket.on(
        "requestBundleChangeCollection",
        function (homebrewID, toAdd, keyCode = "None") {
          if (toAdd) {
            if (keyCode.length > 50) {
              return;
            }
            UserHomebrew.addToHomebrewCollection(
              userID,
              homebrewID,
              keyCode,
            ).then((isSuccess) => {
            ...
            UserHomebrew.removeFromHomebrewCollection(userID, homebrewID).then(
              (isSuccess) => {
                socket.emit("returnBundleChangeCollection", toAdd, isSuccess);
              },
            );
      ...
      socket.on(
        "requestBundleAddKeys",
        function (homebrewID, amount, isOneTimeUse) {
          UserHomebrew.addBundleKeys(
            userID,
            homebrewID,
            amount,
            isOneTimeUse,
      ...
      socket.on("requestBundleRemoveKey", function (homebrewID, keyCode) {
        UserHomebrew.removeBundleKey(userID, homebrewID, keyCode).then(
          (result) => {
    ...
    io.on("connection", function (socket) {
      ...
      socket.on("requestHomebrewUpdateClass", function (homebrewID, data) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            if (data != null && data.classID != null) {
              HomebrewCreation.deleteClass(homebrewID, data.classID).then(
                (result) => {
                  HomebrewCreation.addClass(homebrewID, data).then((result) => {
                    socket.emit("returnHomebrewCompleteClass");
                  });
      ...
      socket.on("requestHomebrewClassDetails", function (homebrewID) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            GeneralGathering.getAllClasses(userID, homebrewID).then(
              (classObject) => {
                GeneralGathering.getAllFeats(userID, homebrewID).then(
                  (featsObject) => {
                    socket.emit(
                      "returnHomebrewClassDetails",
                      classObject,
                      featsObject,
                    );
                  },
      ...
      socket.on("requestHomebrewUpdateAncestry", function (homebrewID, data) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            if (data != null && data.ancestryID != null) {
              HomebrewCreation.deleteAncestry(homebrewID, data.ancestryID).then(
                (result) => {
                  HomebrewCreation.addAncestry(homebrewID, data).then(
                    (result) => {
                      socket.emit("returnHomebrewCompleteAncestry");
                    },
                  );
      ...
      socket.on(
        "requestHomebrewRemoveAncestry",
        function (homebrewID, ancestryID) {
          UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
            if (canEdit) {
              HomebrewCreation.deleteAncestry(homebrewID, ancestryID).then(
                (result) => {
                  socket.emit("returnHomebrewRemoveContent");
                },
              );
      ...
      socket.on("requestHomebrewAncestryDetails", function (homebrewID) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            GeneralGathering.getAllAncestries(userID, true, homebrewID).then(
              (ancestryObject) => {
                GeneralGathering.getAllFeats(userID, homebrewID).then(
                  (featsObject) => {
                    socket.emit(
                      "returnHomebrewAncestryDetails",
                      ancestryObject,
                      featsObject,
                    );
                  },
      ...
      socket.on("requestHomebrewUpdateArchetype", function (homebrewID, data) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            if (data != null && data.archetypeID != null) {
              HomebrewCreation.deleteArchetype(
                homebrewID,
                data.archetypeID,
              ).then((result) => {
                HomebrewCreation.addArchetype(homebrewID, data).then(
                  (result) => {
                    socket.emit("returnHomebrewCompleteArchetype");
                  },
                );
      ...
      socket.on(
        "requestHomebrewRemoveArchetype",
        function (homebrewID, archetypeID) {
          UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
            if (canEdit) {
              HomebrewCreation.deleteArchetype(homebrewID, archetypeID).then(
                (result) => {
                  socket.emit("returnHomebrewRemoveContent");
                },
              );
      ...
      socket.on("requestHomebrewArchetypeDetails", function (homebrewID) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            GeneralGathering.getAllArchetypes(userID, homebrewID).then(
              (archetypeArray) => {
                GeneralGathering.getAllFeats(userID, homebrewID).then(
                  (featsObject) => {
                    socket.emit(
                      "returnHomebrewArchetypeDetails",
                      archetypeArray,
                      featsObject,
                    );
                  },
      ...
      socket.on("requestHomebrewUpdateBackground", function (homebrewID, data) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            if (data != null && data.backgroundID != null) {
              HomebrewCreation.deleteBackground(
                homebrewID,
                data.backgroundID,
              ).then((result) => {
                HomebrewCreation.addBackground(homebrewID, data).then(
                  (result) => {
                    socket.emit("returnHomebrewCompleteBackground");
                  },
                );
      ...
      socket.on(
        "requestHomebrewRemoveBackground",
        function (homebrewID, backgroundID) {
          UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
            if (canEdit) {
              HomebrewCreation.deleteBackground(homebrewID, backgroundID).then(
                (result) => {
                  socket.emit("returnHomebrewRemoveContent");
                },
              );
      ...
      socket.on("requestHomebrewBackgroundDetails", function (homebrewID) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            GeneralGathering.getAllBackgrounds(userID, homebrewID).then(
              (backgrounds) => {
      ...
      socket.on("requestHomebrewAddClassFeature", function (homebrewID, data) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            HomebrewCreation.addClassFeature(homebrewID, data).then(
              (result) => {
      ...
      socket.on(
        "requestHomebrewUpdateClassFeature",
        function (homebrewID, data) {
          UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
            if (canEdit) {
              if (data != null && data.classFeatureID != null) {
                HomebrewCreation.deleteClassFeature(
                  homebrewID,
                  data.classFeatureID,
                ).then((result) => {
                  HomebrewCreation.addClassFeature(homebrewID, data).then(
      ...
      socket.on(
        "requestHomebrewRemoveClassFeature",
        function (homebrewID, classFeatureID) {
          UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
            if (canEdit) {
              HomebrewCreation.deleteClassFeature(
                homebrewID,
      ...
      socket.on("requestHomebrewUpdateFeat", function (homebrewID, data) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            if (data != null && data.featID != null) {
              HomebrewCreation.deleteFeat(homebrewID, data.featID).then(
                (result) => {
                  HomebrewCreation.addFeat(homebrewID, data).then((result) => {
                    socket.emit("returnHomebrewCompleteFeat");
                  });
      ...
      socket.on("requestHomebrewAddHeritage", function (homebrewID, data) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            HomebrewCreation.addHeritage(homebrewID, null, data).then(
              (result) => {
      ...
      socket.on("requestHomebrewUpdateHeritage", function (homebrewID, data) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            if (data != null && data.heritageID != null) {
              HomebrewCreation.deleteHeritage(homebrewID, data.heritageID).then(
                (result) => {
                  HomebrewCreation.addHeritage(homebrewID, null, data).then(
                    (result) => {
                      socket.emit("returnHomebrewCompleteHeritage");
      ...
      socket.on(
        "requestHomebrewRemoveHeritage",
        function (homebrewID, heritageID) {
          UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
            if (canEdit) {
              HomebrewCreation.deleteHeritage(homebrewID, heritageID).then(
                (result) => {
                  socket.emit("returnHomebrewRemoveContent");
                },
              );
      ...
      socket.on("requestHomebrewHeritageDetails", function (homebrewID) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            GeneralGathering.getAllHeritages(userID, homebrewID).then(
              (heritages) => {
                GeneralGathering.getAllAncestriesBasic(userID).then(
                  (ancestries) => {
                    socket.emit(
                      "returnHomebrewHeritageDetails",
                      heritages,
                      ancestries,
                    );
                  },
      ...
      socket.on(
        "requestHomebrewUpdateUniHeritage",
        function (homebrewID, data) {
          UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
            if (canEdit) {
              if (data != null && data.uniHeritageID != null) {
                HomebrewCreation.deleteUniHeritage(
                  homebrewID,
                  data.uniHeritageID,
                ).then((result) => {
                  HomebrewCreation.addUniHeritage(homebrewID, data).then(
                    (result) => {
                      socket.emit("returnHomebrewCompleteUniHeritage");
                    },
                  );
      ...
      socket.on(
        "requestHomebrewRemoveUniHeritage",
        function (homebrewID, uniHeritageID) {
          UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
            if (canEdit) {
              HomebrewCreation.deleteUniHeritage(
                homebrewID,
      ...
      socket.on("requestHomebrewUniHeritageDetails", function (homebrewID) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            GeneralGathering.getAllUniHeritages(userID, homebrewID).then(
              (uniheritages) => {
                GeneralGathering.getAllFeats(userID, homebrewID).then(
                  (featsObject) => {
                    socket.emit(
                      "returnHomebrewUniHeritageDetails",
                      uniheritages,
                      featsObject,
                    );
                  },
      ...
      socket.on("requestHomebrewUpdateItem", function (homebrewID, data) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            if (data != null && data.itemID != null) {
              HomebrewCreation.deleteItem(homebrewID, data.itemID).then(
                (result) => {
                  HomebrewCreation.addItem(homebrewID, data).then((result) => {
                    socket.emit("returnHomebrewCompleteItem");
                  });
      ...
      socket.on("requestHomebrewUpdateSpell", function (homebrewID, data) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            if (data != null && data.spellID != null) {
              HomebrewCreation.deleteSpell(homebrewID, data.spellID).then(
                (result) => {
                  HomebrewCreation.addSpell(homebrewID, data).then((result) => {
                    socket.emit("returnHomebrewCompleteSpell");
                  });
      ...
      socket.on("requestHomebrewSpellDetails", function (homebrewID) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            GeneralGathering.getAllSpells(userID, homebrewID).then(
              (spellMap) => {
        ...
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            if (data != null && data.languageID != null) {
              HomebrewCreation.deleteLanguage(homebrewID, data.languageID).then(
                (result) => {
                  HomebrewCreation.addLanguage(homebrewID, data).then(
                    (result) => {
                      socket.emit("returnHomebrewCompleteLanguage");
                    },
                  );
      ...
      socket.on(
        "requestHomebrewRemoveLanguage",
        function (homebrewID, languageID) {
          UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
            if (canEdit) {
              HomebrewCreation.deleteLanguage(homebrewID, languageID).then(
                (result) => {
                  socket.emit("returnHomebrewRemoveContent");
                },
              );
      ...
      socket.on("requestHomebrewLanguageDetails", function (homebrewID) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            GeneralGathering.getAllLanguages(userID, homebrewID).then(
              (languages) => {
        ...
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            if (data != null && data.traitID != null) {
              HomebrewCreation.deleteTrait(homebrewID, data.traitID).then(
                (result) => {
                  HomebrewCreation.addTrait(homebrewID, data).then((result) => {
                    socket.emit("returnHomebrewCompleteTrait");
                  });
        ...
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            if (data != null && data.toggleableID != null) {
              HomebrewCreation.deleteToggleable(
                homebrewID,
                data.toggleableID,
              ).then((result) => {
                HomebrewCreation.addToggleable(homebrewID, data).then(
                  (result) => {
                    socket.emit("returnHomebrewCompleteToggleable");
                  },
                );
      ...
      socket.on(
        "requestHomebrewRemoveToggleable",
        function (homebrewID, toggleableID) {
          UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
            if (canEdit) {
              HomebrewCreation.deleteToggleable(homebrewID, toggleableID).then(
                (result) => {
                  socket.emit("returnHomebrewRemoveContent");
                },
              );
      ...
      socket.on("requestHomebrewToggleableDetails", function (homebrewID) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            GeneralGathering.getAllToggleables(userID, homebrewID).then(
              (toggleables) => {
        ...
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            if (data != null && data.creatureID != null) {
              HomebrewCreation.deleteCreature(homebrewID, data.creatureID).then(
                (result) => {
                  HomebrewCreation.addCreature(homebrewID, data).then(
                    (result) => {
                      socket.emit("returnHomebrewCompleteCreature");
                    },
                  );
      ...
      socket.on(
        "requestHomebrewRemoveCreature",
        function (homebrewID, creatureID) {
          UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
            if (canEdit) {
              HomebrewCreation.deleteCreature(homebrewID, creatureID).then(
                (result) => {
                  socket.emit("returnHomebrewRemoveContent");
                },
              );
      ...
      socket.on("requestHomebrewCreatureDetails", function (homebrewID) {
        UserHomebrew.canEditHomebrew(userID, homebrewID).then((canEdit) => {
          if (canEdit) {
            GeneralGathering.getAllCreatures(userID, homebrewID).then(
              (creatures) => {
