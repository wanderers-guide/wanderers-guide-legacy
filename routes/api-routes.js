...
router.get('/oauth2/authorize/:id', charAuthCheck, (req, res) => {
  ...
  if(req.query.client_id != null && req.query.response_type != null){
    ...
    if(responseType != 'code'){ res.sendStatus(400); return; }
    ...
    .then((apiClient) => {
      ...
      .then((character) => {
        if(apiClient != null && character != null){
          ...
          res.render('pages/api_request_access', { title: "Request Access - Wanderer's Guide",
            user: req.user,
            apiClient: apiClient,
...
router.post('/oauth2/token', (req, res) => {
  ...
  if(req.query.code != null && req.query.client_id != null && req.header('Authorization') != null){
    ...
    ClientAPI.generateAccessToken(code, clientID, apiKey).then((accessTokenData) => {
...
const charCheck = (req, res, next) => {
  if(req.header('Authorization') != null){
    ...
    ClientAPI.hasAccessToCharacter(req.charID, accessToken).then((accessRights) => {
