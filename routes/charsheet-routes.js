    ...
    .then((character) => {
      if (character != null) {
        ...
        AuthCheck.canViewCharacter(userID, character.id).then((canViewChar) => {
          if (canViewChar) {
            AuthCheck.canEditCharacter(userID, character.id).then(
              (canEditChar) => {
                if (CharStateUtils.isPlayable(character)) {
                  goToCharSheet(character, req, res, !canEditChar, sheetType);
                } else {
                  res.redirect(
                    "/profile/characters/builder/basics/?id=" + character.id,
                  );
                }
              },
            );
            ...
            res.render("error/private_character_error", {
              title: "Private Character Error - Wanderer's Guide",
              user: req.user,
            });
          }
        });
